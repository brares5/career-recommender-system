...
public class GetAllJobsInfo {
    public static void main(String[] args) throws OWLOntologyCreationException {
        ...
        OntClass automationEngineerClass = model.getOntClass("http://www.crs.com/ontologies/crs.owl#AutomationEngineer");
        ...
        if (analystClass != null) {
            ...
            // Extract the educational fields
            StmtIterator educationalFields = analystClass.listProperties(model.getProperty("http://www.crs.com/ontologies/crs.owl#hasEducationalField"));
            StmtIterator educationalFields2 = analystClass.listProperties();
            System.out.println(educationalFields2.toList());
            while (educationalFields.hasNext()) {
                Resource educationalField = educationalFields.nextStatement().getResource();
                String educationalFieldName = educationalField.getPropertyResourceValue(model.getProperty("http://www.crs.com/ontologies/crs.owl#ed_field_name")).toString();
                System.out.println("Educational Field: " + educationalFieldName);
            }

            // Extract the skills
            StmtIterator skills = analystClass.listProperties(model.getProperty("http://www.crs.com/ontologies/crs.owl#hasSkill"));
            while (skills.hasNext()) {
                Resource skill = skills.nextStatement().getResource();
                String skillName = skill.getLocalName();
